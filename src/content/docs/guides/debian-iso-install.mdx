...
I am not running a hypervisor such as Proxmox since I want to run kubernetes bare-metal in Debian. So I'll need
...
2. Go to [Debian distribution page](https://www.debian.org/distrib/netinst) and click on `amd64` since that's the architecture for our N100 Intel chip to download the ISO file.
...
7. Finally, let's add the ISO file to the disk. Change the file name to be the name of your iso file along with the disk name.
    ```bash
    sudo dd if=./debian-12.5.0-amd64-netinst.iso of=/dev/disk4 bs=1m
    ```
